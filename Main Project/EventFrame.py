...
class EventFrame:
    ...
    def __init__(self,
        ...
        self._overallEventFrame = tk.Frame(self._parentFrame, background=self._staticAppInfo.getColor(self._frameDescriptor))
        ...
        self._titleFrame = tk.Frame(self._overallEventFrame, background=self._staticAppInfo.getColor(self._frameDescriptor))
        ...
        if len(eventType) > 0:
            if eventDescriptor == eventType:
                self._title = ttk.Label(self._titleFrame,
                ...
                self._title = ttk.Label(self._titleFrame,
                                        text = "Enter \"" + eventType.lower() + "\" " + eventDescriptor + "s below:",
                                        foreground = self._staticAppInfo.getColor("Default"),
                                        background = self._staticAppInfo.getColor(self._frameDescriptor),
                                        font=self._titleFont)
        else:
    ...
    def buttonFrameSetup(self):
        ...
        if self._eventDescriptor == self._eventType:
            self._setDefaultButton = ttk.Button(self._titleFrame,
                                      text = "Set Default " +
        ...
        #Add at beginning
        self._addButton = ttk.Button(self._titleFrame,
                                  text = "+",
        ...
        #Remove all
        self._removeAllButton = ttk.Button(self._titleFrame,
                                  text = "-",
                                  command = self.removeAllEvents)
        ...
        self._entryFrame = tk.Frame(self._overallEventFrame,
                                    background=self._staticAppInfo.getColor(self._frameDescriptor))
    ...
    def setDefaultEvents(self):
        ...
        otherEvents = self._staticAppInfo.getCopyFullEventsSpecific(self._eventDescriptor)
    ...
    def updateEventEntries(self):
        ...
        self._staticAppInfo.removeEventsFromFullList(list(self._eventWidgets.keys()), self._eventDescriptor)
        ...
        for key in self._eventWidgets:
            ...
            if self._timeEvent:
                self._eventEntries[key] = [self._eventWidgets[key][1][1].get(), self._eventWidgets[key][2][1].get()]
            #Run this to preserve the counter fields if those are available
            if self._counter:
                counterIndex = 0
                for i in range(0, len(self._eventWidgets[key])):
                    if isinstance(self._eventWidgets[key][i], list) and len(self._eventWidgets[key][i]) == 3:
        ...
        for i in range(0, len(self._events)):
            ...
            #Create event label
            eventLabel = ttk.Label(self._entryFrame,
            ...
            if self._timeEvent:
                for k in range(0, 2):
                    ...
                    dropdown = ttk.Combobox(self._entryFrame,
                                            textvariable=selectedOption,
                                            values=options,
                                            state="readonly",
                                            width = 15)
                    dropdown.current(0)
                    self._eventWidgets[self._events[i]].append([dropdown, selectedOption, options])
            ...
            for j in range(0, len(self._eventWidgets[self._events[i]]) - 1):
                if isinstance(self._eventWidgets[self._events[i]][j], tk.Widget):
                    self._eventWidgets[self._events[i]][j].grid(row=i,
                                                                column=j,
                                                                sticky="W",
                                                                padx=5)
            ...
            for eventKey in self._eventWidgets:
                if eventKey in self._counters:
                    ...
                    for i in range(0, len(self._eventWidgets[eventKey])):
                        if (isinstance(self._eventWidgets[eventKey][i], list) and
                                len(self._eventWidgets[eventKey][i]) == 3):
        ...
        self._staticAppInfo.addEventsToFullListSpecific(self._events, self._eventDescriptor)
    ...
    def addEventPopup(self, index):
        ...
        popupFrame = tk.Frame(popup, background=self._staticAppInfo.getColor(self._frameDescriptor))
        ...
        # Create event entry
        eventEntry = ttk.Entry(popupFrame,
        ...
        eventEntry.bind("<Return>", lambda event, i=index, entryField=eventEntry, body=popup, frame=popupFrame:
        self.addEventEntry(i, entryField, body, frame))

        # Create enter button
        enterButton = ttk.Button(popupFrame,
                                 text="Add",
                                 command=lambda i=index, entryField=eventEntry, body=popup, frame=popupFrame:
                                 self.addEventEntry(i, entryField, body, frame))
    ...
    def addEventEntry(self, index, eventEntry, body, frame):
        ...
        # Add the event
        if (not(eventEntry.get() in self._staticAppInfo.getCopyFullEventsSpecific(self._eventDescriptor)) and
        0 < len(eventEntry.get()) <= self._characterLimit and
        eventEntry.get().isalnum()):
    ...
    def submitData(self):
        ...
        if self._timeEvent:
            ...
            for key in self._eventWidgets:
                ...
                if eventError:
                    errors = True
                    widgets[columnError].grid(row = rowCount, column = columnError, sticky = "W", padx = 5)
        ...
        if self._counter:
            for key in self._eventWidgets:
                counterIndex = 0
                for i in range(0, len(self._eventWidgets[key])):
                    if (isinstance(self._eventWidgets[key][i], list) and
                            len(self._eventWidgets[key][i]) == 3):
                        if self._eventWidgets[key][i][2][0] == "1":
                            counterIndex = i
