...
class App:
    ...
    def setUpHomePage(self):
        ...
        openScheduleButton = ttk.Button(self._homeFrame,
                                        ...
                                        command = self.openStandSchedule)
    ...
    def setUpSchedulePage(self):
        ...
        self._scheduleCanvas = tk.Canvas(self._scheduleCanvasFrame, background="#007994")
        scrollbar = ttk.Scrollbar(self._scheduleCanvasFrame,
                                        orient="vertical",
        ...
        self._scheduleCanvas.create_window((0, 0), window=self._scheduleFrame, anchor ="nw")

        #Label for schedule page
        scheduleText = ttk.Label(self._scheduleFrame,
                             text = "Schedule     ",
                             foreground = "Black",
                             background = "#007994",
                             font = ("Arial", 100))
        ...
        #Instruction text
        instructionText = ttk.Label(self._scheduleFrame,
        ...
        self._defaultUpStandButton = ttk.Button(self._scheduleFrame,
                                          text = "Set Default Up Stands",
                                          command = self.setDefaultUpStands)
        self._root.after(100, lambda: self.placeButtonToSide(self._defaultUpStandButton,
                                                             instructionText,
                                                             padx = 7,
                                                             pady = 14))

        #Create button to add an entry
        self._addUpStandButton = ttk.Button(self._scheduleFrame,
                                            text = "+",
                                            command = lambda index = -1 : self.addUpStandPopup(index))
        self._root.after(100, lambda: self.placeButtonToSide(self._addUpStandButton,
                                                             instructionText,
                                                             padx=140,
                                                             pady=14))

        #Create button to remove all entries
        self._removeAllUpStandButton = ttk.Button(self._scheduleFrame,
                                                  text = "-",
                                                  command = self.removeAllUpStands)
        self._root.after(100, lambda: self.placeButtonToSide(self._removeAllUpStandButton,
                                                             instructionText,
                                                             padx=227,
                                                             pady=14))

        #Create second instruction text
        self._secondInstructionText = ttk.Label(self._scheduleFrame,
        ...
        self._timelyDownStandsLabel = ttk.Label(self._scheduleFrame,
        ...
        defaultTimelyDownStandButton = ttk.Button(self._scheduleFrame,
        ...
        addTimelyDownStandButton = ttk.Button(self._scheduleFrame,
                                                         text="+",
                                                         command=self.addTimelyDownStand)
        self._timelyDownStandButtons.append([addTimelyDownStandButton, 195, 11])

        #Creates the button to remove timely down stands
        removeTimelyDownStandButton = ttk.Button(self._scheduleFrame,
                                                         text="-",
                                                         command=self.removeAllTimelyDownStands)
        ...
        self._invisibleLabel = ttk.Label(self._scheduleFrame,
                                   text="",
                                   foreground="Black",
                                   background="#007994",
                                   font=("Arial", 20))
        ...
        #Create back button
        backHomeButton = ttk.Button(self._scheduleFrame,
                                    text = "Back Home",
                                    command = self.backHome)
        self._scheduleButtonsToMoveDown.append([backHomeButton, 0, 7])

        #Create question info button
        questionButton = ttk.Button(self._scheduleFrame,
                                    text="?",
                                    command = self.openScheduleQuestionPopup)
    ...
    def updateTimelyDownStands(self):
        ...
        for key in self._timelyDownStandWidgets:
            self._timelyDownStandEntries[key] = self._timelyDownStandWidgets[key][1].get()
        ...
        for i in range(0, len(self._timelyDownStands)):
            timelyDownStandLabel = ttk.Label(self._scheduleFrame,
                          text="Stand " + self._timelyDownStands[i] + ":",
                          foreground="Black",
                          background="#007994",
                          font=("Arial", 20))
            timelyDownStandEntry = ttk.Entry(self._scheduleFrame,
                          font = self._entryFont)
            addButton = ttk.Button(self._scheduleFrame,
                           text="+",
                           command=lambda index = i: self.addTimelyDownStandEntry(index))
            removeButton = ttk.Button(self._scheduleFrame,
                           text="-",
        ...
        for key in self._timelyDownStandWidgets:
            if key in self._timelyDownStandEntries:
                self._timelyDownStandWidgets[key][1].insert(0, self._timelyDownStandEntries[key])
    ...
    def updateTimelyDownStandsGrid(self):
        ...
        for i in range(0, len(self._timelyDownStands)):
            ...
            self._timelyDownStandWidgets[self._timelyDownStands[i]][0].grid(row=len(self._upStandWidgets) + i + 4,
                                                                            column=0,
                                                                            sticky="W",
                                                                            padx=5)
            self._timelyDownStandWidgets[self._timelyDownStands[i]][1].grid(row=len(self._upStandWidgets) + i + 4,
                                                                            column=1,
                                                                            sticky="W")
            self._timelyDownStandWidgets[self._timelyDownStands[i]][2].grid(row=len(self._upStandWidgets) + i + 4,
                                                                            column=2,
                                                                            sticky="W")
            self._timelyDownStandWidgets[self._timelyDownStands[i]][3].grid(row=len(self._upStandWidgets) + i + 4,
                                                                            column=3,
                                                                            sticky="W")
    ...
    def updateScheduleBottomButtons(self):
        self._secondInstructionText.grid(row=2+len(self._upStandWidgets),
                                         column=0,
                                         padx=5,
                                         pady=0,
                                         sticky="NW",
                                         columnspan = 7)
        self._timelyDownStandsLabel.grid(row=3+len(self._upStandWidgets),
                                         column=0,
                                         padx=5,
                                         pady=0,
                                         sticky="NW",
        ...
        self._invisibleLabel.grid(row=total,
                                  column=0,
                                  padx=5,
                                  pady=0,
                                  sticky="NW")
    ...
    def updatePlaceButtonsBelow(self):
        for button in self._scheduleButtonsToMoveDown:
            self.placeButtonToSide(button[0], self._invisibleLabel, button[1], button[2])
    ...
    def updateTimelyDownStandButtons(self):
        for button in self._timelyDownStandButtons:
            self.placeButtonToSide(button[0], self._timelyDownStandsLabel, button[1], button[2])
    ...
    def overlappingStandError(overlappingStands):
        ...
        #Create error text
        errorText = ttk.Label(popupFrame,
        ...
        overlappingStandsLabel = ttk.Label(popupFrame,
    ...
    def updateUpStandEntries(self):
        ...
        for i in range(0, len(self._upStands)):
            standLabel = ttk.Label(self._scheduleFrame,
                          text="Stand " + self._upStands[i] + ":",
                          foreground="Black",
                          background="#007994",
                          font=("Arial", 20))
            standEntry = ttk.Entry(self._scheduleFrame,
                          font = self._entryFont)
            addButton = ttk.Button(self._scheduleFrame,
                           text="+",
            ...
            #Place buttons
            self._upStandWidgets[self._upStands[i]][0].grid(row =  i + 2,
                                                               column = 0,
                                                               sticky = "W",
                                                               padx = 5)
            self._upStandWidgets[self._upStands[i]][1].grid(row =  i + 2,
                                                               column = 1,
                                                               sticky = "W")
            self._upStandWidgets[self._upStands[i]][2].grid(row =  i + 2,
                                                               column = 2,
                                                               sticky = "W")
            self._upStandWidgets[self._upStands[i]][3].grid(row =  i + 2,
                                                               column = 3,
                                                               sticky = "W")
    ...
    def openScheduleQuestionPopup(self):
        ...
        #Place schedule frame in window
        self._scheduleQuestionCanvas.create_window((0, 0), window=popupFrame, anchor ="nw")

        #Create label for format information
        formatLabel = ttk.Label(popupFrame,
        ...
        formatExample = ttk.Label(popupFrame,
        ...
        formatExplanation = ttk.Label(popupFrame,
        ...
        subtractButtonExplanation = ttk.Label(popupFrame,
        ...
        addButtonExplanation = ttk.Label(popupFrame,
                                     text="-->The one at the top next to the \"Set Default\n"
        ...
        standDifferenceExplanation = ttk.Label(popupFrame,
                                     text="-->The one at the top next to the \"Set Default\n"
                                          "Up Stands\" brings up a popup to add a stand entry\n"
                                          "field. Once the user presses the add button on the\n"
                                          "popup, the entry will be added to the top of the\n"
        ...
        self._scheduleQuestionCanvas.config(scrollregion=self._scheduleQuestionCanvas.bbox("all"))
        self._scheduleQuestionCanvas.bind_all("<MouseWheel>", self.onMouseWheelScheduleQuestion)
    ...
    def addUpStandPopup(self, index):
        ...
        instructionText = ttk.Label(popupFrame,
        ...
        #Create stand entry
        standEntry = ttk.Entry(popupFrame,
                               font=self._entryFont)
        standEntry.grid(row = 1, column = 0, sticky = "W", padx = 5)
        standEntry.bind("<Return>", lambda event, i = index, entryField = standEntry, body = popup, frame = popupFrame:
                                 self.addUpStandEntry(i, entryField, body, frame))

        #Create enter button
        enterButton = ttk.Button(popupFrame,
                                 text = "Add",
                                 command=lambda i = index, entryField = standEntry, body = popup, frame = popupFrame:
                                 self.addUpStandEntry(i, entryField, body, frame))
    ...
    def addUpStandEntry(self, index, standEntry, body, frame):
        ...
        #Add the stand
        alreadyExists = (standEntry.get() in self._timelyDownStandWidgets or
                         standEntry.get() in self._upStandWidgets)
        ...
        #Otherwise display an error message
            ...
            error = ttk.Label(frame,
                              text = "Error, either duplicate\nor more than 2 characters",
                              foreground = "#A50000",
                              background="#007994",
                              font=("Arial", 15))
            error.grid(row = standEntry.grid_info()["row"] + 2,
                       column = 0)
    ...
    def placeButtonToSide(button, widget, padx, pady):
        button.place(x = widget.winfo_x() + widget.winfo_width() + padx,
                     y = widget.winfo_y() + pady)
