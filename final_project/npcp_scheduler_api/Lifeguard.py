...
class Lifeguard:
    ...
    def __init__(
        ...
        self._numBreaks = math.ceil(
            (
                ...
                + StaticAPIInfo.breakTime
            )
            / (StaticAPIInfo.longestTimeWorking + StaticAPIInfo.breakTime)
            - 1
        )
        ...
        for t in range(
            ...
            self._schedule[thisTime] = StaticAPIInfo.emptyCode
        self.updateBreaks()
    ...
    def getFurthestTimeBackToDisruption(self, currentTime):
        ...
        for breakTime in self._breakTimes:
            ...
            timesOfBreaksAndShiftStarts.append(
                breakTime.getMinutes() + StaticAPIInfo.breakTime
            )
    ...
    def getIntervalsUpOnStand(self, thisTime, upStands):
    ...
    def getIntervalsDownOnStand(self, thisTime, upStands):
        # Check the type of the parameter
        if isinstance(thisTime, Time):
            # Get the index of the time given in self._schedule keys
            index = None
    ...
    def updateBreaks(self):
        ...
        for breakTime in self._breakTimes:
            for i in range(0, StaticAPIInfo.getBreakInterval()):
                thisTime = Time().setTimeWithMinutes(
                    breakTime.getMinutes() + i * StaticAPIInfo.timeInterval
                )
                for scheduleTime in self._schedule:
                    if scheduleTime.equals(thisTime):
                        self._schedule[scheduleTime] = StaticAPIInfo.breakCode
    ...
    def calculateRangeOfPossibleBreakTimes(self):
        ...
        earliestTime = Time().setTimeWithMinutes(
            self._endTime.getMinutes()
            - StaticAPIInfo.longestTimeWorking
            - StaticAPIInfo.breakTime
        )
        latestTime = Time().setTimeWithMinutes(
            self._startTime.getMinutes() + StaticAPIInfo.longestTimeWorking
        )
        ...
        for thisTime in self._breakTimes:
            if givenTime.getIsInBetweenExclusiveEnd(
                ...
                Time().setTimeWithMinutes(
                    thisTime.getMinutes() + StaticAPIInfo.breakTime
                ),
            ):
                return True
        return False
    ...
    def resetLifeguardSchedule(self, resetTime: Time = Time(0, 0)):
        ...
        for scheduleTime in self._schedule:
            if scheduleTime.getMinutes() >= resetTime.getMinutes():
                self._schedule[scheduleTime] = StaticAPIInfo.emptyCode
    ...
    def getUpStandsFromTime(self, givenTime: Time, upStands: list[Stand | str]) -> int:
        ...
        upStandCode = StaticAPIInfo.upStandCode
        ...
        while stand is not None and stand in upStandNames:
            stand = self.getStand(
                Time().setTimeWithMinutes(
                    givenTime.getMinutes() + StaticAPIInfo.timeInterval * (count + 1)
                )
            )
    ...
    def convertScheduleToUp(self, upStands: list[Stand], branchTime: Time):
        ...
        for scheduleTime in self._schedule:
            if (
                ...
                self._schedule[scheduleTime] = StaticAPIInfo.upStandCode
