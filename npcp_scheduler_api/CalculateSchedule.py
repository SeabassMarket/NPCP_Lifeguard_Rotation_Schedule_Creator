...
class CalculateSchedule:
    ...
    def optimizeUpcomingBreaks(self, currentTime):
        ...
        nextTime = Time().setTimeWithMinutes(
            currentTime.getMinutes() + StaticAPIInfo.timeInterval
        )
        ...
        for lifeguard in lifeguardsGoingOnBreak:
            ...
            while timeBeingAnalyzed >= timeFurthestBack.getMinutes():
                ...
                for lifeguardNotGoingToBreak in lifeguardsWorkingAtTime:
                    listToAdd.append(
                        lifeguardNotGoingToBreak.getIntervalsUpOnStand(
                            timeObjectBeingAnalyzed, self.getUpStandNames()
                        )
                    )
                ...
                timesAndIntervalsUpOnStandGoingOnBreak[timeBeingAnalyzed] = (
                    lifeguard.getIntervalsUpOnStand(
                        timeObjectBeingAnalyzed, self.getUpStandNames()
                    )
                )

                # Decrease the time by the interval time
                timeBeingAnalyzed -= StaticAPIInfo.timeInterval
        ...
        for lifeguard in uniqueLifeguardsThatCanBeSwappedWith:
            ...
            lifeguardGoingOnBreakUpStandIntervalScore[lifeguard] = (
                lifeguard.getIntervalsUpOnStand(nextTime, self.getUpStandNames())
            )
            ...
            for i in range(0, len(listOfIndices)):
                ...
                intervalsUpOnStand = lifeguardsWorkingAtTime[
                    index
                ].getIntervalsUpOnStand(nextTime, self.getUpStandNames())
                lifeguardsScores.append(
                    intervalsUpOnStand
                    - lifeguard.getIntervalsUpOnStand(nextTime, self.getUpStandNames())
                )
                if intervalsUpOnStand <= lifeguard.getIntervalsUpOnStand(
                    nextTime, self.getUpStandNames()
                ):
            ...
            for i in range(0, numIterations):
                ...
                for lifeguard in tempLifeguards:
                    intervalsUpOnStand.append(
                        lifeguard.getIntervalsUpOnStand(
                            currentTime, self.getUpStandNames()
                        )
                    )
                ...
                if tieBreaker and intervalsUpOnStand[0] == 0:
                    ...
                    for lifeguard in tempLifeguards:
                        intervalsDownOnStand.append(
                            lifeguard.getIntervalsDownOnStand(
                                currentTime, self.getUpStandNames()
                            )
                        )
    ...
    def reorganizeLifeguards(self):
        ...
        for t in range(
            earliestTime.getMinutes() - StaticAPIInfo.timeInterval,
            latestTime.getMinutes(),
            StaticAPIInfo.timeInterval,
        ):
            currentTime = Time().setTimeWithMinutes(t)
    ...
    def reorganizeLifeguardsAtTime(
        ...
        def getValidPermutations(limit: int) -> dict[int, list[list[str]]]:
            ...
            permutations = StaticAPIInfo.getStandComboPermutations()
    ...
    def getUpStandNames(self):
        ...
        upStandNames.append(StaticAPIInfo.upStandCode)
    ...
    def getLifeguardsComingFromDownAtTime(
        ...
        for lifeguard in lifeguards:
            if (
                lifeguard.getIntervalsUpOnStand(currentTime, self.getUpStandNames())
                == 0
            ):
    ...
    def getLifeguardsUpOnStandAtSpecificTime(self, currentTime):
        ...
        if isinstance(currentTime, Time):
            ...
            upStands.append(StaticAPIInfo.upStandCode)
    ...
    def calculateAndAssignLeftoverBreaks(self, lifeguardsNeedingBreaks):
        ...
        for lifeguard in lifeguardsNeedingBreaks:
            ...
            timeWithHighestRatio = StaticAPIInfo.findDictMax(lifeguardStandRatios)
    ...
    def calculateBreaks(self):
        ...
        StaticAPIInfo.clipDictionaryToTimeRange(lifeguardsAtTime, possibleBreakRange)
        ...
        StaticAPIInfo.sortTimesAscending(breaks)
    ...
    def getUpStandsAtTimeDict(self):
        ...
        for i in range(0, 1440, StaticAPIInfo.timeInterval):
    ...
    @staticmethod
    def reduceLifeguardScore(lifeguardsAtTime, timeWithHighestRatio):
        # For each interval in the break interval
        for j in range(0, StaticAPIInfo.getBreakInterval()):
            # Create the time to remove from going forward
            timeToRemoveFromForward = Time().setTimeWithMinutes(
                timeWithHighestRatio.getMinutes() + j * StaticAPIInfo.timeInterval
            )
            ...
            for thisTime in lifeguardsAtTime:
                if thisTime.equals(timeToRemoveFromForward):
                    lifeguardsAtTime[thisTime] -= StaticAPIInfo.getBreakInterval() - j
                ...
                for thisTime in lifeguardsAtTime:
                    if thisTime.equals(timeToRemoveFromBackward):
                        lifeguardsAtTime[thisTime] -= (
                            StaticAPIInfo.getBreakInterval() - j
                        )
        ...
        for i in range(0, 1440, StaticAPIInfo.timeInterval):
            ...
            for j in range(0, StaticAPIInfo.getBreakInterval()):
                ...
                timeObserving = Time().setTimeWithMinutes(
                    thisTime.getMinutes() + j * StaticAPIInfo.timeInterval
                )
    ...
    def printSchedule(self):
        ...
        for t in range(
            ...
            # Set the last time
            lastTime = Time().setTimeWithMinutes(t - StaticAPIInfo.timeInterval)

            # Set the next time
            nextTime = Time().setTimeWithMinutes(t + StaticAPIInfo.timeInterval)
            ...
            for lifeguard in self._lifeguards:
                ...
                if stand is None:
                    stand = ""
                elif stand == StaticAPIInfo.upStandCode:
                    stand = "UP"
                    standLength = len(stand)
                elif stand == StaticAPIInfo.breakCode:
                    stand = "\033[93m YYY\033[0m"
                elif stand == StaticAPIInfo.emptyCode:
