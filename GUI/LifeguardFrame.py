...
class LifeguardFrame:
    ...
    def __init__(self, root, motherFrame, homeScreen, staticAppInfo):
        ...
        self._lifeguardCanvas = tk.Canvas(
            self._lifeguardCanvasFrame,
        ...
        self._lifeguardFrame = tk.Frame(
            self._lifeguardCanvas,
            background=self._staticAppInfo.getColor(self._frameDescriptor),
        )

        # Place lifeguard frame in window
        self._lifeguardCanvas.create_window(
            (0, 0), window=self._lifeguardFrame, anchor="nw"
        )

        """THIS MARKS THE END OF CANVAS CREATION AND BEGINNING OF FRAME CREATION"""

        # Title
        self._lifeguardTitle = ttk.Label(
            self._lifeguardFrame,
            text="Lifeguards",
        ...
        self._lifeguardEventFrame = EventFrame(
            self._middleFrame,
            row=1,
            root=root,
        ...
        self._questionButton = ttk.Button(
            bottomFrame, text="?", command=self.openLifeguardQuestionPopup
        )
        self._staticAppInfo.appendButtonToDisable(
        ...
        self._submitButton = ttk.Button(
            bottomFrame, text="Submit", command=self.submitLifeguards
        )
        self._staticAppInfo.appendButtonToDisable(
            self._submitButton
        )  # Append to buttons to disable
    ...
    def submitLifeguards(self):
        ...
        for key in lifeguardData:
            if lifeguardData[key] == "ERROR":
                errors = True
        if not errors:
            self._staticAppInfo.setEventDataSpecific(
                lifeguardData, self._frameDescriptor
            )
    ...
    def openLifeguardQuestionPopup(self):
        ...
        # Set entry and exit data
        self._staticAppInfo.addEnterAndExit(
            name="Lifeguard Question Canvas",
            threeValueList=[
                self._questionPopupFrame,
                self._scheduleQuestionCanvas,
                self._questionPopup,
